...
/*   Updated: 2022/08/01 17:05:13 by nsartral         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	lexer_id_two(t_first **uno, char *str, int *mode, t_garbage **garbage)
{
	...
	{
		add_back_uno(uno, new_uno(APPEND, alloc_content(&str[i - 1] \
			, 2, garbage), garbage));
		*mode = NEUTRAL_MODE;
		return (0);
	}
	if (*mode == R_REDIR_MODE && str[i] != '>')
	{
		add_back_uno(uno, new_uno(WRITE, alloc_content(&str[i - 1] \
			, 1, garbage), garbage));
...
int	lexer_id_three(t_first **uno, char *str, int *mode, t_garbage **garbage)
{
	...
	{
		add_back_uno(uno, new_uno(HEREDOC, alloc_content(&str[i - 1] \
			, 2, garbage), garbage));
		*mode = NEUTRAL_MODE;
		return (0);
	}
	if (*mode == L_REDIR_MODE && str[i] != '<')
	{
		add_back_uno(uno, new_uno(READ, alloc_content(&str[i - 1] \
			, 1, garbage), garbage));
		*mode = actual_mode(str[i]);
	}
	if (str[i] == '|')
	{
		add_back_uno(uno, new_uno(PIPE, alloc_content(&str[i] \
			, 1, garbage), garbage));
		return (0);
	}
	return (1);
